...
#define GPIO_TS_WAKE	CFG_IO_TOUCH_RESET_PIN
...
{
	nxp_soc_gpio_set_out_value(GPIO_TS_WAKE, 0);
	return 0;
}

static int gslX680_shutdown_high(void)
{
	nxp_soc_gpio_set_out_value(GPIO_TS_WAKE, 1);
	return 0;
}
...
static int clr_reg(struct i2c_client *client)
{
	int ret;
	u8 write_buf[4]	= {0};

	write_buf[0] = 0x88;
	ret = gsl_ts_write(client, 0xe0, &write_buf[0], 1); 	
	if(ret < 0)
		return ret;
	//msleep(20);
	write_buf[0] = 0x03;
	ret = gsl_ts_write(client, 0x80, &write_buf[0], 1); 
	if(ret < 0)
		return ret;	
	//msleep(5);
	write_buf[0] = 0x04;
	ret = gsl_ts_write(client, 0xe4, &write_buf[0], 1);
	if(ret < 0)
		return ret; 	
	//msleep(5);
	write_buf[0] = 0x00;
...
static int gslX680_ts_init(struct i2c_client *client, struct gsl_ts *ts)
{
    ...
    input_mt_init_slots(input_device,6);
#ifdef HAVE_TOUCH_KEY
