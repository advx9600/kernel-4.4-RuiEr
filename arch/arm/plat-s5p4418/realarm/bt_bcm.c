...
#define	BT_GPIO_REG_ON			(PAD_GPIO_C + 14)//(PAD_GPIO_C + 7)
#define	BT_GPIO_WAKE_DEVICE 	(PAD_GPIO_B + 9)//(PAD_GPIO_B + 24)

#define	BT_GPIO_WAKE_HOST 		(-1)//(PAD_GPIO_D + 20)
...
static  void bt_bcm_set_io_val(struct bt_ctl_gpio *bti, int on)
{
	...
	pr_debug("bt_bcm: bt_bcm_set_io_val (%s.%d) [%s]\n",
...
#define	UART1_CTS_ALT_CTS	2
#define	UART1_CTS_ALT_IO	1
...
static int bt_bcm_rfkill_set_block(void *data, bool blocked)
{
	...
	pr_debug("bt_bcm: rfkill set_block %s %s.%d\n",
