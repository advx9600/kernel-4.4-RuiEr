...
int  nxpmac_init(struct platform_device *pdev)
{
    ...
    nxp_soc_gpio_set_io_func(PAD_GPIO_E+ 7, 1);  
    nxp_soc_gpio_set_io_func(PAD_GPIO_E+ 8, 1); 
    nxp_soc_gpio_set_io_func(PAD_GPIO_E+ 9, 1); 
    nxp_soc_gpio_set_io_func(PAD_GPIO_E+ 10, 1); 
    nxp_soc_gpio_set_io_func(PAD_GPIO_E+ 11, 1); 
    nxp_soc_gpio_set_io_func(PAD_GPIO_E+ 14, 1); 
    nxp_soc_gpio_set_io_func(PAD_GPIO_E+ 15, 1); 
    nxp_soc_gpio_set_io_func(PAD_GPIO_E+ 16, 1); 
    nxp_soc_gpio_set_io_func(PAD_GPIO_E+ 17, 1); 
    nxp_soc_gpio_set_io_func(PAD_GPIO_E+ 18, 1); 
    nxp_soc_gpio_set_io_func(PAD_GPIO_E+ 19, 1); 
    nxp_soc_gpio_set_io_func(PAD_GPIO_E+ 20, 1); 
    nxp_soc_gpio_set_io_func(PAD_GPIO_E+ 21, 1); 
    nxp_soc_gpio_set_io_func(PAD_GPIO_E+ 22, 1);
    nxp_soc_gpio_set_io_func(PAD_GPIO_E+ 23, 1);
    nxp_soc_gpio_set_io_func(PAD_GPIO_E+ 24, 1);
...
#elif defined (CONFIG_REALTEK_PHY_RTL8211)	// 1000Base-T
...
static struct plat_stmmacenet_data nxpmac_plat_data = {
    ...
    .speed = SPEED_100,
#else
    .phy_addr = 4,// 7 for 8211 3 for 8201
    .clk_csr = 0x4,
    .speed = SPEED_1000,
#endif
 ...
 * realarm gpio config
 */
#define CONFIG_REALARM_BOARD_GPIO
#ifdef CONFIG_REALARM_BOARD_GPIO
void realarm_gpio_config()
{
	/*wifi bt*/
	nxp_soc_gpio_set_io_func(CFG_WIFI_POWER_IO, 1); //set wifi gpio alt
	nxp_soc_gpio_set_io_dir(CFG_WIFI_POWER_IO, 1);
    nxp_soc_gpio_set_io_func(CFG_WIFI_ON_IO, 1); //set wifi gpio alt
    nxp_soc_gpio_set_io_dir(CFG_WIFI_ON_IO, 1);
    
   /* nxp_soc_gpio_set_io_func(CFG_WL_HOST_WAKE, 1); //
    nxp_soc_gpio_set_io_dir(CFG_WL_HOST_WAKE, 0);
    nxp_soc_gpio_set_int_enable(CFG_WL_HOST_WAKE, 1);*/
	/*LCD*/
	nxp_soc_gpio_set_io_func(CFG_LCD_ENABLE,1);
	nxp_soc_gpio_set_io_dir(CFG_LCD_ENABLE, 1);  
    nxp_soc_gpio_set_out_value(CFG_LCD_ENABLE, 1);
    /*CTP*/
    nxp_soc_gpio_set_io_func(CFG_IO_TOUCH_RESET_PIN,1);
    nxp_soc_gpio_set_io_dir(CFG_IO_TOUCH_RESET_PIN, 1); 
    nxp_soc_gpio_set_out_value(CFG_IO_TOUCH_RESET_PIN, 0);
    mdelay(20);
    nxp_soc_gpio_set_out_value(CFG_IO_TOUCH_RESET_PIN, 1);
	/*GPS*/	
    nxp_soc_gpio_set_io_func(CFG_GPS_ENABLE,1);
    nxp_soc_gpio_set_io_dir(CFG_GPS_ENABLE, 1);
    nxp_soc_gpio_set_out_value(CFG_GPS_ENABLE, 1);
    
    /*4G*/
    printk("enable_4G \n");
	//reset pin
	nxp_soc_gpio_set_io_func(PAD_GPIO_B + 26,1);
	nxp_soc_gpio_set_io_dir(PAD_GPIO_B + 26, 1);
    nxp_soc_gpio_set_out_value(PAD_GPIO_B + 26, 0);
 
	msleep(100);
	//power pin
	nxp_soc_gpio_set_io_func(PAD_GPIO_C + 17,1);
	nxp_soc_gpio_set_io_dir(PAD_GPIO_C + 17, 1);
    nxp_soc_gpio_set_out_value(PAD_GPIO_C + 17, 1);
...
#if defined(CONFIG_SND_SOC_ES8396)
#include <linux/i2c.h>

/* CODEC */
static struct i2c_board_info __initdata es8396_i2c_bdi = {
	.type	= "es8396-codec",			// compatilbe with wm8976
	.addr	= 0x11,		// 0x1A (7BIT), 0x34(8BIT)
};

/* DAI */
struct nxp_snd_dai_plat_data es8396_i2s_dai_data = {
	.i2s_ch	= 0,
	.sample_rate	= 48000,
	.hp_jack 		= {
		.support    	= 1,
...
static struct platform_device es8396_dai = {
	...
	.id				= 0,
	.dev			= {
		.platform_data	= &es8396_i2s_dai_data,
	}
};
#endif
...
/* gpio i2c 3 */
#define	I2C3_SCL	PAD_GPIO_C + 16
#define	I2C3_SDA	PAD_GPIO_C + 15
...
static int front_camera_power_enable(bool on)
{
    ...
    if (on) {
        ...
        if (!is_front_camera_enabled) {
			...
			nxp_soc_gpio_set_out_value(io, 0);
            nxp_soc_gpio_set_out_value(reset_io, 0);
            nxp_soc_gpio_set_out_value(CFG_IO_CAMERA_FRONT_POWER, 1);
	    	mdelay(10);
            /* PD signal */
            nxp_soc_gpio_set_out_value(io, 1);
            mdelay(6);
        ...
        if (is_front_camera_enabled) {
            nxp_soc_gpio_set_out_value(CFG_IO_CAMERA_FRONT_POWER, 0);
            nxp_soc_gpio_set_out_value(reset_io, 0);
            nxp_soc_gpio_set_out_value(io, 0);
...
static struct i2c_board_info front_camera_i2c_boardinfo[] = {
 ...
 //       I2C_BOARD_INFO("SP0838", 0x18),
        ...
        I2C_BOARD_INFO("GM7150", 0xb8>>1),//0xba
    },
};
...
int s5p4418_mcp2515_init(struct spi_device *spi)
{
	printk("ERTER:%s ......\n",__func__);
    ...
    nxp_soc_gpio_set_io_func(PAD_GPIO_C+ 28,  0); 
...
void __init nxp_board_devices_register(void)
{
...
#endif
#if defined(CONFIG_REALARM_BOARD_GPIO)
    printk("plat: enable realarm board gpio control\n");
    realarm_gpio_config();
#endif
#if defined(CONFIG_MMC_DW)
	#ifdef CONFIG_MMC_SLSI_CH0
    ...
    printk("plat: enable wifi power\n");
    ap6210_pm_gpio_ctrl(1);
    ap6210_wifi_on_ctrl(1);
    #endif
	...
	i2c_register_board_info(0, &es8396_i2c_bdi, 1);
